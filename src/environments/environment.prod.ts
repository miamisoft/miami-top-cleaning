export const environment = {
  production: true,
  defaultLan: 'en',
  cookieLan: 'MIA-TOP-CL-LAN',
  cookieQuotePopup: 'MIA-TOP-CL-QUOTE_POPUP',
  wpApiBaseUrl: 'https://miamitopcleaning.com/wp-json/wp',
  phpApiUrl: 'https://miamitopcleaning.com/server/api',
  dataBaseUrl: '/assets/data',
  facebookUrl: 'https://www.facebook.com/miamitopcleaning/',
  googleMapUrl: 'https://www.google.com/maps/place/Miami+Top+Cleaning+Service/@25.748514,-80.312378,14z/data=!4m5!3m4!1s0x0:0x701da3604f8bc3c9!8m2!3d25.748514!4d-80.312378?hl=en',
  yelpUrl: 'https://www.yelp.com/biz/miami-top-cleaning-service-miami',
  youtubeUrl: 'https://www.youtube.com/channel/UCzE6m0YERbTadS-Nr_1UcgQ',
  twitterUrl: 'https://twitter.com/miamitopcleanin'
};
